...
def train():
    model=dense.TCN(input_size=4,output_size=4, num_channels= [12,12,24,32,48,64] , kernel_size=7, dropout=0.3) #这句话是用之前定义好的类SeismicNet，生成model对象
...
#            loss=F.nll_loss(output,target)
            loss=F.nll_loss(output,target,weight=Variable(torch.Tensor([2.69,1.06,2.06,1]).cuda()))

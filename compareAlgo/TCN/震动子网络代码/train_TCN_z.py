...
import DataSet_z #读取分帧后的数据
import densezd7 #网络模型
...
from temp import model_lasted_path,lasted,weights_123 #得到.pth文件 设置保存路径
...
lr=0.002
def load_data():
    trainset=DataSet_z.DataSet()#调用类，生成对象
...
def train():
    model=densezd7.TCN(input_size=1,output_size=4, num_channels= [12,12,24,32,48,64] , kernel_size=7, dropout=0.2) #这句话是用之前定义好的类SeismicNet，生成model对象

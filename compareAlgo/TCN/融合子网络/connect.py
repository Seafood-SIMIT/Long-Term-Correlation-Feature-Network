...
old_path='/home/fanyuying/two_streams_data/未降噪训练集特征'
...
for j in tqdm(range(len(files))):
    ...
    if ednfix[0] == 'A':
        if files[j].endswith('.txt'):
            datas=np.loadtxt(files[j])   #[1,2,3,,,n]  [[1,2,3,4],[5,6,7,8],,,[]]
            ...
            data_z = np.loadtxt(S_name)
            data_z = np.reshape(data_z,[-1,1])
            print(datas.shape)
            print(data_z.shape)

          #  data = data_s+data
            #data = np.concatenate([data_s,data],axis=1)
            data = np.concatenate([data_z,datas],axis=1)
            print(data.shape)

            savepath =  os.path.join('/home/fanyuying/two_streams_data/未降噪训练集融合特征_相加','[1+1'+ ednfix[1:])

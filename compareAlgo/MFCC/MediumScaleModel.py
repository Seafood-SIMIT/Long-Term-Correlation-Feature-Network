...
def yuchuli_semi(data_semi):
    #data=data[::4]
    data_semi=data_semi-np.mean(data_semi)
    data_semi = data_semi/np.max(np.abs(data_semi))
    data_semi = torch.from_numpy(np.array(data_semi)).float().view(1,-1)
    return Variable(data_semi.unsqueeze(0))
...
#model2 原始数据1024维
        self.features=nn.Sequential(
            ...
            nn.Conv1d(64, class_num, kernel_size=4, stride=1,bias=True),#nxclass_numx1
            )
